...
using Ozon.BaseClassLibrary;
using Ozon.Library;
using Ozon.Library.CommonExtensions;
using Ozon.Library.StringExtensions;
using Ozon.PageFlowClassLibrary;
using Ozon.PageFlowClassLibrary.Cms;
using Ozon.PageFlowClassLibrary.Menu;
using Ozon.Shop.Common;
using Umbraco.Core;
using Umbraco.Core.Models;
using UmbracoCMS.AppCode.Interfaces;
using UmbracoCMS.AppCode.Manager;
using UmbracoCMS.AppCode.Model;
...
namespace UmbracoCMS.AppCode
{
	...
	{
		#region Properties
		private readonly int _allGoodsFacetId = CommonFunctions.GetSafeValue<int>(ConfigurationManager.AppSettings["AllGoodsFacetId"]);
		...
		private static readonly int HorizontalMenuItemsCount =
ConfigurationManager.AppSettings["HorizontalMenuItemsCount"].To<int>();
			...
			switch (alias) {
				case "name": {
						...
						if (int.TryParse(propValue, out preValueId) && preValueId > 0) {
							sectionProperty.Name = ApplicationContext.Current.Services.DataTypeService.GetPreValueAsString(preValueId);
						} else {
		...
		private void ConvertFromArchetypeModelToAbTestInnerUmbracoSwitch(string alias, AbTestInnerUmbraco abtest, string propValue)
		{
			switch (alias) {
				...
				case "ids": {
					abtest.AbGroups = ArchetypeConverterManager.ConvertFromArchetypeModelToListT<AbGroup>(propValue, ConvertFromArchetypeModelToAbGroup);
						break;
					}
			}
		}
			...
			if (HorizontalMenuBySecondLevel.Contains(webSection.Name)) {
				...
				if (extendedMenu != null) {
					items =
						...
						where !item.Href.IsNullOrEmpty() && !item.Items.IsEmpty()
						select new HorizontalMenuItemInfo {
							Name = item.Name,
							Href = item.Href,
						};
		...
		public CmsPageInfo GetPage(string param, ref ErrorInfo error, CmsPageSource source)
		{
			...
			// шаблон
			var template = CommonManager.GetCurrentTemplate(UnifiedContent.GetTemplateId(content));
			...
			if (currentAreaCompliance.Any()) {
				areasBackofice = currentAreaCompliance.Select(a => new CmsAreaInfo {
					...
					Name = a.BackofficeSource.LastOrDefault(),
			...
			if (parents.Count > 2) {
				...
				pageInfoFromOzon.ParentsAdequate = pageParent.Select(p=> p.PageParent).ToList();
				pageParent.Reverse();
